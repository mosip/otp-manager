...
if [ "$ACTION" == "upgrade" ]; then
  ...
  if [ -f "$UPGRADE_SCRIPT_FILE" ]; then
    ...
    PGPASSWORD=$SU_USER_PWD psql -v ON_ERROR_STOP=1 -v dbuserpwd="'${DBUSER_PWD}'" --username=$SU_USER --host=$DB_SERVERIP --port=$DB_PORT --dbname=$DEFAULT_DB_NAME -v primary_language_code=$PRIMARY_LANGUAGE_CODE -a -b -f $UPGRADE_SCRIPT_FILE
  else
