...
then
    ...
    # --- safe properties loader (replaces the original while loop) ---
    while IFS='=' read -r key value || [ -n "$key" ]; do
      # skip empty lines and comments
      [[ -z "$key" ]] && continue
      case "$key" in \#*) continue ;; esac

      # trim whitespace around key and value, convert dots to underscores
      key=$(echo "$key" | tr '.' '_' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
      value=$(echo "$value" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')

      # assign the value to a variable with the name in $key, without further expansion
